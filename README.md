...
- **点数字作答**：不弹键盘。点「时」弹出 1–12 的 **4 列 × 3 排**面板；点「分」弹出**两组并排**的面板——左边十位 `0–5`（3 列 × 2 排）、右边个位 `0–9`（5 列 × 2 排），两组按钮同宽同高，拼出 00–59。
  - 分钟面板**点十位不收起**（还要接着点个位，个位数字保持不变），**点个位就算选完、自动收起**；所以「先十位再个位」两下就完成，改个位只要一下。
  - 点面板外、按 Esc、再点同一个格子也能收起。作答后数字位锁死。
  - 面板在下方放不下时会**自动向上翻**；窄屏（≤900px）贴屏幕底部弹出，保证整块都能看见。
- **随机出题**：点「随机出一道题」生成一个时间（不与上一题重复），页面打开时自动出一道。
...
- 左栏只有一张卡片：**时钟在上，讲解按钮和讲解内容紧贴其下**，看指针和看讲解不用来回找。
- 右栏：作答区（出题按钮、时/分数字位 + 弹出面板、提交、反馈）。
